...
- XCT001 だけは日本語のキーで、大半にスペースが入る（半角。マニュアルの表は全角で描画されます）
...
**実数型は値が整数だと `int` でデコードされます。** JSON に数値型が1つしかないためで、XKT016 の `A48_012` は `1275.0` ではなく `1275` で届きます。それでも `float` と注釈します。typing の仕様が `float` に `int` を受けさせるので算術は正しく動き、`int | float` にすると全利用箇所に絞り込みを強いるだけだからです。

代償は2つあります。`isinstance(value, float)` が `False` になること、それと float 専用の操作を報告するかがチェッカー次第になることです。`ty` は `float` を `int | float` としてモデル化するので `properties["A48_012"].hex()` をエラーにしますが、mypy 1.18 は注釈をそのまま `float` と読んで通し、実行時に `AttributeError` になります。`py.typed` を同梱しているので、**どちらになるかは利用者のチェッカーが決めます。**
...
`＜パラメータ＞` の表と違い、`＜出力＞` の表はクライアント側で描画されます。表示されたテキストからの転記では空白が失われるので、**生の HTML から取ってください。** RSC の flight payload の中に
...
### 実レスポンスとの照合は全35本済んでいます

**各エンドポイントを1回ずつ叩いて突き合わせました。** マニュアルと食い違ったときは実レスポンスを採っています。

| 確認したこと | 結果 |
|---|---|
| キー名 | **XCT001 だけが違った。109個のうち63個。** 区切りは U+3000 ではなく半角スペース、`用途区分コード`→`用途区分`、`構造コード`→`構造`、`建蔽率`→`建ぺい率`、`緯度`→`位置座標 緯度`。実キーに差し替え済み。他34本は一致 |
| `proximity_to_transportation_facilitites` の綴り | マニュアル通り。API 側の綴り違いで確定 |
| 宣言された値の型 | 35本すべて一致。実数型が `int` で届く件は[値型の節](#値型はマニュアルの宣言に従います)に書きました |
| `DataResponse` の封筒 | `status` / `data` で一致。`status` は `"OK"`。他の値を取らない確証はないので `Literal` にはしていない |
| `null` で来るフィールド | **35本で1件もなかった。** nullability を表現しない判断は維持 |
| キーの省略 | 値がないときは基本的に空文字。**欠落するのは2本だけ**で、XKT007 の学校分類系3フィールドと XKT013 の `GASSAN20XX`。`total=False` は正しかった |
| ジオメトリ種別 | **7本が同一タイルで複数の形を返す。** XKT001・014・020・026・027 は Polygon と MultiPolygon、XKT030 は LineString と MultiLineString、XKT029 は Polygon・MultiPolygon・LineString の3つ、XST001 は Polygon と Point。さらに XKT015（駅別乗降客数）は LineString のみ。エンドポイント別に狭めることはできない |
| `crs` | **2種類あった。** XKT017（図書館）と XKT019（自然公園地域）だけ EPSG:4612（JGD2000）、残り30本は EPSG:6668（JGD2011）。**座標を混ぜる前に読むべき値です** |
| `name` | マニュアルにないが全32本にある。31本が文字列、XPT001 だけ配列。XKT001 は2レイヤをカンマ連結した1文字列 |
| `_id` / `_index` | マニュアルにない。Elasticsearch のメタデータで、キー順の先頭。タイル系31本が持ち、XPT001 だけ持たない |
| XKT007 の2つの形 | **マニュアルの2表は実物と違う。** 全フィーチャが両方のフィールドを持ち、幼稚園側は福祉施設コードが空文字。本当に条件付きなのは学校分類系3フィールドだけ。1つの型のままにした |
| XKT013 の年 | 2020〜2070 の5年刻みだが、2020は `PTN_2020` のみ、`GASSAN` は2055年以降のみで欠落もある。公表される推計次第で動くので `dict[str, Any]` のまま |

締める方向（`total=False` → 必須、合併型を狭める、`Literal` にする）は読み取り側にとって非破壊です。広げる方向（`None` を足す）は破壊的なので、`feat!:` が必要になります。

**`total=False` は外していません。** 1エンドポイント1クエリの標本で、しかも XKT007 と XKT013 で実際に欠落が出ました。締めるのはいつでも非破壊にできるので急ぐ理由がありません。

データが更新されたときは `scripts/` の2本で再確認できます。
...
1本目が35本のマニュアルページとレスポンスを `.reinfolib-responses/` に落とし、2本目が突き合わせて食い違いを出します。**リクエストは各マニュアルページの curl 例をそのまま使います。** 適当なタイルを選ぶと空のフィーチャ一覧が返ってきて、キーの有無という一番知りたいことが分からないためです。マニュアルが更新されれば座標も向こうが直すので、こちらに35本分のパラメータを持ちません。

2本目はネットワークもAPIキーも使わず、出力はキー名と型だけです（値そのものが答えになる `status`・ジオメトリの `type`・`name`・`crs` は例外）。
