...
### 実レスポンスとの照合は17本まで済んでいます

**照合済みは XIT001・XIT002・XCT001・XPT001・XPT002・XKT001〜007・XKT010・XKT011・XKT013〜015 の17本です。** 残りはマニュアルの `＜出力＞` 表だけを根拠にしています。**マニュアルは実物と食い違うことがある**ので、実レスポンスを読めたエンドポイントはそちらを採ってください。

済んだ確認と結果です。

| 確認したこと | 結果 |
|---|---|
| XCT001 のキー | **109個のうち63個が違った。** 区切りは U+3000 ではなく半角スペース、`用途区分コード`→`用途区分`、`構造コード`→`構造`、`建蔽率`→`建ぺい率`、`緯度`→`位置座標 緯度`。実キーに差し替え済み |
| 他16本のキー | マニュアル通り。食い違ったのは XCT001 だけ |
| `proximity_to_transportation_facilitites` の綴り | マニュアル通り。API 側の綴り違いで確定 |
| 宣言された値の型 | 17本すべて一致。文字列型・整数型・実数型・真偽型の対応は正しい |
| `DataResponse` の封筒 | `status` / `data` で一致。`status` は `"OK"`。他の値を取らない確証はないので `Literal` にはしていない |
| `null` で来るフィールド | **1件もなかった。** nullability を表現しない判断は維持 |
| キーの省略 | 値がないときは基本的に空文字。**ただし例外が2本あり**、XKT007 の学校系3フィールドと XKT013 の `GASSAN20XX` は実際に欠落する。`total=False` は正しかった |
| ジオメトリ種別 | **XKT001 と XKT014 は同一タイルに Polygon と MultiPolygon が混在。** さらに XKT015（駅別乗降客数）は LineString。エンドポイント別に狭めることはできない |
| `name` / `crs` | どちらもマニュアルにないが全レスポンスにある。`crs` は全本 EPSG:6668（JGD2011）。`name` は型が不統一で `str \| list[str]` |
| `_id` / `_index` | マニュアルにない。Elasticsearch のメタデータで、キー順の先頭。タイル系14本が持ち、XPT001 だけ持たない |
...
| 確認すること | 違っていた場合 |
|---|---|
| XKT016〜031、XGT001、XST001 のキー | マニュアルとの差分。XCT001 の前例があるので楽観できません |

締める方向（`total=False` → 必須、合併型を狭める、`Literal` にする）は読み取り側にとって非破壊です。広げる方向（`None` を足す）は破壊的なので、`feat!:` が必要になります。

**`total=False` は外していません。** 17本で確認しても1エンドポイント1クエリの標本で、しかも XKT007 と XKT013 で実際に欠落が出ました。締めるのはいつでも非破壊にできるので急ぐ理由がありません。
