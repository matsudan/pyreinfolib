...
5. **括弧の中身がデータの限定なら残す。** 洪水浸水想定区域（想定最大規模）→ `expected_flood_inundation_areas_at_maximum_scale`。手順3の言い換えでも手順4のデータセット名でもなく、同名のデータのうちどれかを絞る語です。落とすと何が返るか名前から分からなくなります
6. **定型辞を落とす。** 情報、取得、一覧、API
