...
データが更新されたときの再確認は同じ手順です。[出力表はサーバ描画されていません](#出力表はサーバ描画されていません)でマニュアル側を取り、各エンドポイントのマニュアルページにある curl 例の座標でレスポンスを取ります。**その座標を使うのが肝心で**、適当なタイルを選ぶと空のフィーチャ一覧が返ってきて、キーの有無という一番知りたいことが分かりません。
