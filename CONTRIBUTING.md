...
[GLOSSARY.md](GLOSSARY.md) で訳し、**日本語のコード表記を代入の直後に docstring として置きます。**
...
コメントではなく docstring にするのは、**エディタがホバーと補完で表示するのはこの位置の文字列だけ**だからです。メンバー名は英語なので、`LAND` と `LAND_AND_BUILDING` のどちらが 宅地(土地) かは名前から決まりません。利用者がそれを知りたいのは、コードを書いている最中です。

日本語はコード表の表記をそのまま写します。全角と半角の括弧が混在していますが（`宅地(土地)` と `林地（都道府県地価調査）`）、揃えないでください。原文と一致していることが、写し間違いを見つける唯一の手がかりです。

実行時には取れません。`PriceClassification.CONTRACT_PRICE.__doc__` はクラスの docstring を返します。日本語を実行時に使いたくなったら `__new__` で `label` 属性を持たせる方法がありますが、公開する属性が増えるので、必要になってから足してください。後から足すのは非破壊的です。
